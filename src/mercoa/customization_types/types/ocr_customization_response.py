# This file was auto-generated by Fern from our API Definition.

from ...core.pydantic_utilities import UniversalBaseModel
import typing_extensions
from ...core.serialization import FieldMetadata
import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2
import typing


class OcrCustomizationResponse(UniversalBaseModel):
    """
    Examples
    --------
    from mercoa.customization_types import OcrCustomizationResponse

    OcrCustomizationResponse(
        line_items=True,
        collapse_line_items=True,
        invoice_metadata=True,
        line_item_metadata=True,
        line_item_gl_account_id=True,
        predict_metadata=True,
        tax_and_shipping_as_line_items=True,
        split_documents=True,
    )
    """

    line_items: typing_extensions.Annotated[bool, FieldMetadata(alias="lineItems")] = pydantic.Field()
    """
    Extract line items from the invoice. Defaults to true.
    """

    collapse_line_items: typing_extensions.Annotated[bool, FieldMetadata(alias="collapseLineItems")] = pydantic.Field()
    """
    If true, the line items will be collapsed into a single line item. Defaults to false.
    """

    invoice_metadata: typing_extensions.Annotated[bool, FieldMetadata(alias="invoiceMetadata")] = pydantic.Field()
    """
    Pull custom metadata at the invoice level. Defaults to true.
    """

    line_item_metadata: typing_extensions.Annotated[bool, FieldMetadata(alias="lineItemMetadata")] = pydantic.Field()
    """
    Pull custom metadata at the line item level. Defaults to true.
    """

    line_item_gl_account_id: typing_extensions.Annotated[bool, FieldMetadata(alias="lineItemGlAccountId")] = (
        pydantic.Field()
    )
    """
    Pull GL Account ID at the line item level. Defaults to true.
    """

    predict_metadata: typing_extensions.Annotated[bool, FieldMetadata(alias="predictMetadata")] = pydantic.Field()
    """
    Use AI to predict metadata from historical data. Defaults to true.
    """

    tax_and_shipping_as_line_items: typing_extensions.Annotated[
        bool, FieldMetadata(alias="taxAndShippingAsLineItems")
    ] = pydantic.Field()
    """
    Pull tax and shipping information as line items. Defaults to true. If false, tax and shipping will extracted as invoice level fields.
    """

    split_documents: typing_extensions.Annotated[bool, FieldMetadata(alias="splitDocuments")] = pydantic.Field()
    """
    Use AI to intelligently split documents into multiple subdocuments. Defaults to false.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
