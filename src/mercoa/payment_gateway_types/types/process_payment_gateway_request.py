# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations
from ...core.pydantic_utilities import UniversalBaseModel
import typing
import typing_extensions
from .process_payment_gateway_card_details import ProcessPaymentGatewayCardDetails
from ...core.serialization import FieldMetadata
from ...core.pydantic_utilities import IS_PYDANTIC_V2
import pydantic


class ProcessPaymentGatewayRequest_Document(UniversalBaseModel):
    """
    Examples
    --------
    from mercoa.payment_gateway_types import (
        ProcessPaymentGatewayCardDetails_Direct,
        ProcessPaymentGatewayRequest_Document,
    )

    ProcessPaymentGatewayRequest_Document(
        document="data:application/pdf;base64,",
        card_details=ProcessPaymentGatewayCardDetails_Direct(
            first_name="John",
            last_name="Doe",
            card_number="4242424242424242",
            expiration_month=10,
            expiration_year=2025,
            cvv="123",
            postal_code="12345",
            country="US",
        ),
    )
    """

    type: typing.Literal["document"] = "document"
    document: str
    card_details: typing_extensions.Annotated[ProcessPaymentGatewayCardDetails, FieldMetadata(alias="cardDetails")]

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


class ProcessPaymentGatewayRequest_Html(UniversalBaseModel):
    """
    Examples
    --------
    from mercoa.payment_gateway_types import (
        ProcessPaymentGatewayCardDetails_Direct,
        ProcessPaymentGatewayRequest_Document,
    )

    ProcessPaymentGatewayRequest_Document(
        document="data:application/pdf;base64,",
        card_details=ProcessPaymentGatewayCardDetails_Direct(
            first_name="John",
            last_name="Doe",
            card_number="4242424242424242",
            expiration_month=10,
            expiration_year=2025,
            cvv="123",
            postal_code="12345",
            country="US",
        ),
    )
    """

    type: typing.Literal["html"] = "html"
    html: str
    card_details: typing_extensions.Annotated[ProcessPaymentGatewayCardDetails, FieldMetadata(alias="cardDetails")]

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


"""
from mercoa.payment_gateway_types import (
    ProcessPaymentGatewayCardDetails_Direct,
    ProcessPaymentGatewayRequest_Document,
)

ProcessPaymentGatewayRequest_Document(
    document="data:application/pdf;base64,",
    card_details=ProcessPaymentGatewayCardDetails_Direct(
        first_name="John",
        last_name="Doe",
        card_number="4242424242424242",
        expiration_month=10,
        expiration_year=2025,
        cvv="123",
        postal_code="12345",
        country="US",
    ),
)
"""
ProcessPaymentGatewayRequest = typing.Union[ProcessPaymentGatewayRequest_Document, ProcessPaymentGatewayRequest_Html]
